...
(*
    ...
    member _.onAnimationCancel (handler: AnimationEvent -> unit) = h.MakeEvent("onAnimationCancel", handler)

    /// Fires when animation ends.
    member _.onAnimationEnd (handler: AnimationEvent -> unit) = h.MakeEvent("onAnimationEnd", handler)

    /// Fires when animation iterates.
    member _.onAnimationIteration (handler: AnimationEvent -> unit) = h.MakeEvent("onAnimationIteration", handler)

    /// Fires when animation starts.
    member _.onAnimationStart (handler: AnimationEvent -> unit) = h.MakeEvent("onAnimationStart", handler)

    /// Fires the moment that the element loses focus.
    member _.onBlur (handler: FocusEvent -> unit) = h.MakeEvent("onBlur", handler)
    ...
    member _.onChange (handler: bool -> unit) = h.MakeEvent("onChange", (fun (ev: Event) -> handler (!!ev.target?``checked``)))
    ...
    member _.onChange (handler: string -> unit) = h.MakeEvent("onChange", (fun (ev: Event) -> handler (!!ev.target?value)))
    ...
    member _.onCheckedChange (handler: bool -> unit) = h.MakeEvent("onChange", (fun (ev: Event) -> handler (!!ev.target?``checked``)))

    /// Fires on a mouse click on the element.
    member _.onClick (handler: MouseEvent -> unit) = h.MakeEvent("onClick", handler)

    /// Fires when composition ends.
    member _.onCompositionEnd (handler: CompositionEvent -> unit) = h.MakeEvent("onCompositionEnd", handler)

    /// Fires when composition starts.
    member _.onCompositionStart (handler: CompositionEvent -> unit) = h.MakeEvent("onCompositionStart", handler)

    /// Fires when composition changes.
    member _.onCompositionUpdate (handler: CompositionEvent -> unit) = h.MakeEvent("onCompositionUpdate", handler)

    /// Fires when a context menu is triggered.
    member _.onContextMenu (handler: MouseEvent -> unit) = h.MakeEvent("onContextMenu", handler)
    ...
    member _.onCopy (handler: ClipboardEvent -> unit) = h.MakeEvent("onCopy", handler)

    /// Fires when the user cuts the content of an element.
    member _.onCut (handler: ClipboardEvent -> unit) = h.MakeEvent("onCut", handler)

    /// Fires when a mouse is double clicked on the element.
    member _.onDblClick (handler: MouseEvent -> unit) = h.MakeEvent("onDblClick", handler)

    /// Fires when an element is dragged.
    member _.onDrag (handler: DragEvent -> unit) = h.MakeEvent("onDrag", handler)

    /// Fires when the a drag operation has ended.
    member _.onDragEnd (handler: DragEvent -> unit) = h.MakeEvent("onDragEnd", handler)

    /// Fires when an element has been dragged to a valid drop target.
    member _.onDragEnter (handler: DragEvent -> unit) = h.MakeEvent("onDragEnter", handler)

    member _.onDragExit (handler: DragEvent -> unit) = h.MakeEvent("onDragExit", handler)

    /// Fires when an element leaves a valid drop target.
    member _.onDragLeave (handler: DragEvent -> unit) = h.MakeEvent("onDragLeave", handler)

    /// Fires when an element is being dragged over a valid drop target.
    member _.onDragOver (handler: DragEvent -> unit) = h.MakeEvent("onDragOver", handler)

    /// Fires when the a drag operation has begun.
    member _.onDragStart (handler: DragEvent -> unit) = h.MakeEvent("onDragStart", handler)

    /// Fires when dragged element is being dropped.
    member _.onDrop (handler: DragEvent -> unit) = h.MakeEvent("onDrop", handler)
    ...
    /// Fires when an error occurs.
    member _.onError (handler: UIEvent -> unit) = h.MakeEvent("onError", handler)

    /// Fires the moment when the element gets focus.
    member _.onFocus (handler: FocusEvent -> unit) = h.MakeEvent("onFocus", handler)

    /// Fires when an element captures a pointer.
    member _.onGotPointerCapture (handler: PointerEvent -> unit) = h.MakeEvent("onGotPointerCapture", handler)
    ...
    member _.onKeyDown (handler: KeyboardEvent -> unit) = h.MakeEvent("onKeyDown", handler)
    ...
    /// Fires when a user presses a key.
    member _.onKeyPress (handler: KeyboardEvent -> unit) = h.MakeEvent("onKeyPress", handler)
    ...
    member _.onKeyUp (handler: KeyboardEvent -> unit) = h.MakeEvent("onKeyUp", handler)
    ...
    member _.onLostPointerCapture (handler: PointerEvent -> unit) = h.MakeEvent("onLostPointerCapture", handler)

    /// Fires when a mouse button is pressed down on an element.
    member _.onMouseDown (handler: MouseEvent -> unit) = h.MakeEvent("onMouseDown", handler)

    /// Fires when a pointer enters an element.
    member _.onMouseEnter (handler: MouseEvent -> unit) = h.MakeEvent("onMouseEnter", handler)

    /// Fires when a pointer leaves an element.
    member _.onMouseLeave (handler: MouseEvent -> unit) = h.MakeEvent("onMouseLeave", handler)

    /// Fires when the mouse pointer is moving while it is over an element.
    member _.onMouseMove (handler: MouseEvent -> unit) = h.MakeEvent("onMouseMove", handler)

    /// Fires when the mouse pointer moves out of an element.
    member _.onMouseOut (handler: MouseEvent -> unit) = h.MakeEvent("onMouseOut", handler)

    /// Fires when the mouse pointer moves over an element.
    member _.onMouseOver (handler: MouseEvent -> unit) = h.MakeEvent("onMouseOver", handler)

    /// Fires when a mouse button is released while it is over an element.
    member _.onMouseUp (handler: MouseEvent -> unit) = h.MakeEvent("onMouseUp", handler)

    /// Fires when the user pastes some content in an element.
    member _.onPaste (handler: ClipboardEvent -> unit) = h.MakeEvent("onPaste", handler)
    ...
    member _.onPointerCancel (handler: PointerEvent -> unit) = h.MakeEvent("onPointerCancel", handler)

    /// Fires when a pointer becomes active.
    member _.onPointerDown (handler: PointerEvent -> unit) = h.MakeEvent("onPointerDown", handler)

    /// Fires when a pointer is moved into an elements boundaries or one of its descendants.
    member _.onPointerEnter (handler: PointerEvent -> unit) = h.MakeEvent("onPointerEnter", handler)

    /// Fires when a pointer is moved out of an elements boundaries.
    member _.onPointerLeave (handler: PointerEvent -> unit) = h.MakeEvent("onPointerLeave", handler)

    /// Fires when a pointer moves.
    member _.onPointerMove (handler: PointerEvent -> unit) = h.MakeEvent("onPointerMove", handler)
    ...
    member _.onPointerOut (handler: PointerEvent -> unit) = h.MakeEvent("onPointerOut", handler)

    /// Fires when a pointer is moved into an elements boundaries.
    member _.onPointerOver (handler: PointerEvent -> unit) = h.MakeEvent("onPointerOver", handler)

    /// Fires when a pointer is no longer active.
    member _.onPointerUp (handler: PointerEvent -> unit) = h.MakeEvent("onPointerUp", handler)
    ...
    member _.onResize (handler: UIEvent -> unit) = h.MakeEvent("onResize", handler)
    ...
    member _.onSelect (handler: UIEvent -> unit) = h.MakeEvent("onSelect", handler)
    ...
    /// instead of extracting it from the event arguments.
    member _.onTextChange (handler: string -> unit) = h.MakeEvent("onChange", (fun (ev: Event) -> handler (!!ev.target?value)))
    ...
    member _.onTouchCancel (handler: TouchEvent -> unit) = h.MakeEvent("onTouchCancel", handler)

    member _.onTouchEnd (handler: TouchEvent -> unit) = h.MakeEvent("onTouchEnd", handler)

    member _.onTouchMove (handler: TouchEvent -> unit) = h.MakeEvent("onTouchMove", handler)

    member _.onTouchStart (handler: TouchEvent -> unit) = h.MakeEvent("onTouchStart", handler)

    member _.onTransitionEnd (handler: TransitionEvent -> unit) = h.MakeEvent("onTransitionEnd", handler)
    ...
    member _.onWheel (handler: WheelEvent -> unit) = h.MakeEvent("onWheel", handler)
*)
